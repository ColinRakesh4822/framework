...
function RetrieveComponents()
	...
	Config = exports["vertex-base"]:FetchComponent("Config")
	Sounds = exports["vertex-base"]:FetchComponent("Sounds")
	Execute = exports["vertex-base"]:FetchComponent("Execute")
	Waitlist = exports["vertex-base"]:FetchComponent("WaitList")
	RegisterChatCommands()
end

AddEventHandler("Core:Shared:Ready", function()
	exports["vertex-base"]:RequestDependencies("Commands", {
		"Chat",
		"Callbacks",
		"Config",
		"Sounds",
...
function RegisterChatCommands()
	...
	-- 	local data = exports["vertex-base"]:FetchComponent("Punishment"):Kick(tonumber(args[1]), args[2], source)
	...
	-- 			exports["vertex-base"]:FetchComponent("Punishment"):Kick(t:GetData("Source"), args[2], source)
	-- 		else
	...
	Chat:RegisterAdminCommand("unban", function(source, args, rawCommand)
		exports["vertex-base"]:FetchComponent("Punishment").Unban:BanID(args[1], source)
	end, {
		help = "Unban Player",
		params = {
			{
	...
	Chat:RegisterAdminCommand("unbanid", function(source, args, rawCommand)
		...
		if type == "identifier" then
			exports["vertex-base"]:FetchComponent("Punishment").Unban:Identifier(args[2], source)
		elseif type == "account" then
			exports["vertex-base"]:FetchComponent("Punishment").Unban:AccountID(tonumber(args[2]), source)
		end
	end, {
		help = "Unban Site ID",
		params = {
	...
	-- 			exports["vertex-base"]:FetchComponent("Punishment").Ban:Source(targetSource, days, args[3], source)
	-- 		else
	...
	-- 			if (days >= 1 and days <= 7) or (player.Permissions:IsAdmin() and days >= -1 and days <= 90) then
	-- 				exports["vertex-base"]:FetchComponent("Punishment").Ban:Source(t:GetData("Source"), days, args[3], source)
	-- 			else
	-- 				Chat.Send.System:Single(source, "Invalid Time")
	-- 			end
	-- 		else
	...
	Chat:RegisterAdminCommand("banid", function(source, args, rawCommand)
		...
		if player then
			...
			if days >= -1 and days <= 90 then
				...
				elseif type == "account" then
					local res =
						exports["vertex-base"]:FetchComponent("Punishment").Ban:AccountID(target, days, args[4], source)
					if res and res.success then
